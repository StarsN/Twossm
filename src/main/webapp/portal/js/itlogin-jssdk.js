/**接入参数**/
var appid = "300011910869";
var version = "1.2";//版本信息
var protocol = window.location.protocol + "//";//协议
//var flag= false;//取号是否成功标志位
var mobile="";//保存一键登录取到的加密手机号
var param = {};
/**接入参数**/
...
function getPhoneNumber(appType, openType) {
    $.ajax({
        url:  "/sjsc/intelligentlink/intelligentLinkAction!sign.action",
        data: {'sign': YDRZ.getSign(appid, version)},
        type: 'post',
        dataType: 'json',
        success: function (result) {
            if (result.code == 0) {
                console.log("取号功能开启")
                getToken(result.data.data.sign, result.data.data.msgid, appType, openType);
            } else if(result.code == -2){
                console.log("取号功能未开启")
                $('#one_click_login').hide();
            }else{
                console.log("准签名加签失败")
            }
        },
        error: function () {
            var res = {"result": false, "code": "-1", "desc": "error occur during get sign"};
            //  func(res);
        }
    });
}

function getToken(sign, msgId, appType, openType) {
    YDRZ.getTokenInfo({
        data: {//请求的参数
            version: version, //接口版本号 （必填）
            appId: appid, //应用Id （必填）
            sign: sign,//RSA加密后的sign（必填）
            openType: openType, //移动取号接口填写1,三网取号接口填写0 （必填）
        ...
        success: function (result) {//成功回调
            if (result.code == "000000") {
                ...
                param.userInformation = result.userInformation;
                param.msgId = msgId;
                param.appType = appType;
             //   validateToken(result.token, result.userInformation, msgId, appType);
            } else {
                param.mark = false;
                console.log(result.code+':'+result.message);
            }

        },    //获取浏览器指纹后请在进行一遍urlencode
        error: function (result) {//错误回调
            console.log(result.code+':'+result.message);
        }
    });
...
function validateToken(token, userInformation, msgId, appType,index) {
    $.ajax({
        url: "/sjsc/intelligentlink/intelligentLinkAction!achieveInfo.action",
        data: {'token': token, 'userInformation': userInformation, 'msgId': msgId, 'appType': appType},
        type: 'post',
        dataType: 'json',
        success: function (result) {
            if(result.data.data.code == "103000"){
                mobile = result.data.data.msisdn;
                //flag = true;
                toLogin(mobile,index);
               /* if(flag){
                    //取号成功则显示一键登录按钮
                    $('#one_click_login').show();
                }*/
            }
        },
        error: function () {
            //var res = {"result": false, "code": "-1", "desc": "error occur during validate token"};
            console.log(result.code +':'+ result.message);
            layer.close(index);
            layer.open({content: "信息校验失败，请稍后重试或换种登陆方式", skin: "msg", time: 2});
